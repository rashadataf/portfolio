...
if [ "$NODE_ENV" = "production" ] && [ "$WITH_CERTBOT" = "true" ]; then
  envsubst '$$DOMAIN_NAME$$PORT$$GRAFANA_PORT' < /etc/nginx/templates/production.conf.template > /etc/nginx/conf.d/default.conf
  nginx &
  sleep 15
  ...
  if [ "$NODE_ENV" = "production" ]; then
    ...
    envsubst '$$DOMAIN_NAME$$PORT$$GRAFANA_PORT' < /etc/nginx/templates/dev.conf.template > /etc/nginx/conf.d/default.conf
  fi
