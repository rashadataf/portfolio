...
// URL schema that accepts BOTH absolute URLs (https://...) and relative
// paths ("/api/files/...") generated by the file upload feature.
// z.url() alone rejects relative URLs, which silently broke saves after
// uploading a file (resume, hero image, project image).
const urlOrRelativeSchema = z
  .string()
  .refine(
    (value) => {
      if (value === "") return true; // empty is allowed (field cleared)
      try {
        new URL(value); // absolute URL
        return true;
      } catch {
        return value.startsWith("/") && !value.includes(".."); // relative path, no traversal
      }
    },
    { message: "Invalid URL" }
  );
...
export const createProjectSchema = z.object({
  ...
  // Accepts relative URLs like "/api/files/..." (from file upload) or absolute URLs
  imageUrl: urlOrRelativeSchema,
  technologies: z.array(z.string()).min(1, "At least one technology is required"),
  liveUrl: urlOrRelativeSchema.optional(),
  sourceCodeUrl: urlOrRelativeSchema.optional(),
  playStoreUrl: urlOrRelativeSchema.optional(),
  appStoreUrl: urlOrRelativeSchema.optional(),
...
export const updateProfileSchema = z.object({
  ...
  resumeUrl: urlOrRelativeSchema.optional(),
  contactEmail: z.email("Invalid email").optional().or(z.literal("")),
  heroImageUrl: urlOrRelativeSchema.optional(),
});
